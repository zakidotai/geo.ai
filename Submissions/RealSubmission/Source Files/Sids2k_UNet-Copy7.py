...
# depth of UNet = 4
learning_rate = 2e-4
torch.manual_seed(10069)
ia.imgaug.seed(10069)
epoch_number = 20
batch_size = 64 
train_size = 100 
aug_train_size = 500
...
def augmenter(image, segmap, index):
    ...
    if(mode == 1):
        image, segmap = iaa.Sequential([
            iaa.WithBrightnessChannels(iaa.Add((-5, 5))),
        ...
        image, segmap = iaa.Sequential([
            iaa.Sharpen(alpha=(0.4, 0.6), lightness=(0.8, 1.5)),
        ...
        image, segmap = iaa.Sequential([
            iaa.Fliplr(p=0.1),
            iaa.Flipud(p=0.8),
            iaa.Affine(scale=(0.9,1.2),rotate=(-30,30)),
            iaa.PerspectiveTransform(scale=(0.01, 0.15)),
            iaa.WithHueAndSaturation([
                iaa.WithChannels(0, iaa.Add((-20, 20))),
                iaa.WithChannels(1, [
                    iaa.Multiply((0.55, 1.35)),
                    iaa.LinearContrast((0.75, 1.15))
                ])
            ])
        ])(image=image, segmentation_maps=segmap)
    elif(mode == 5):
...
class DataSet(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        if(self.mode == 'Train'):
            bad_images = ['6J02','4J02','K360','J754','J430','J374','H902','H363']
            bad_images = list(map(lambda x : x+'.jpg', bad_images))
            image = plt.imread(os.path.join(os.getcwd(),'Data_ML','image_chips',self.image_names[index%train_size]))
            if(self.image_names[index%train_size] in bad_images):
                mask = fiona.open(os.path.join(os.getcwd(),'Data_ML','target_feature_AOI',self.image_names[index%train_size][:-4]+'.shp'))
                coordinates = mask[0]['geometry']['coordinates']
                point_path = Path(coordinates[0])
            else:
...
for epoch in range(epochs):  
    ...
    for inputs, labels in tqdm(train_dataLoader): 
        ...
        loss = loss_func(outputs, labels) + torch.norm(layer.weight)
        loss.backward() 
    ...
    for inputs, labels in tqdm(valid_dataLoader): 
        ...
        outputs = Model(inputs) 
        outputs = torchvision.transforms.Resize((750,750))(outputs)
        valloss = loss_func(outputs, labels)+ torch.norm(layer.weight)
